...
public class ArchiveRepo {
    ...
    public ArchiveRepo(Context context){
...
//    }
    ...
    public ArrayList<HashMap<String, String>> getNodeList(){
        ...
        String selectQuery="SELECT "+
                ...
                DEMO_NODE.KEY_name+","+
                DEMO_NODE.KEY_level+","+
                DEMO_NODE.KEY_c_node+","+
                DEMO_NODE.KEY_p_node+","+
                DEMO_NODE.KEY_content +
                " FROM "+DEMO_NODE.TABLE;
        ...
        if(cursor.moveToFirst()){
            do{
                ...
                Nodes.put("nodeid",cursor.getString(cursor.getColumnIndex(DEMO_NODE.KEY_id)));
                Nodes.put("name",cursor.getString(cursor.getColumnIndex(DEMO_NODE.KEY_name)));
                NodesList.add(Nodes);
    ...
    public DEMO_NODE getNodeById(int curNodeId){
        db=SQLiteDatabase.openOrCreateDatabase(DBManager.DB_PATH + "/" + DBManager.DB_NAME, null);
        ...
        String selectQuery="SELECT "+
                ...
                DEMO_NODE.KEY_c_node + "=?";
        int iCount=0;
        DEMO_NODE demoNode=null;
        Cursor cursor=db.rawQuery(selectQuery,new String[]{String.valueOf(curNodeId)});
        if(cursor.moveToFirst()){
            do{
                demoNode=new DEMO_NODE();
                demoNode.id =cursor.getInt(cursor.getColumnIndex(DEMO_NODE.KEY_id));
                demoNode.name =cursor.getString(cursor.getColumnIndex(DEMO_NODE.KEY_name));
                demoNode.c_node  =cursor.getInt(cursor.getColumnIndex(DEMO_NODE.KEY_c_node));
                demoNode.p_node =cursor.getInt(cursor.getColumnIndex(DEMO_NODE.KEY_p_node));
                demoNode.level =cursor.getInt(cursor.getColumnIndex(DEMO_NODE.KEY_level));
                demoNode.content  =cursor.getString(cursor.getColumnIndex(DEMO_NODE.KEY_content));
    ...
    public List<DEMO_NODE> getNodesBylevel(int level){
        db=SQLiteDatabase.openOrCreateDatabase(DBManager.DB_PATH + "/" + DBManager.DB_NAME, null);
        ...
        List<DEMO_NODE> listNODES=new ArrayList<>();
        if(cursor.moveToFirst()){
            do{
                demoNode=new DEMO_NODE();
                demoNode.id =cursor.getInt(cursor.getColumnIndex(DEMO_NODE.KEY_id));
    ...
    public List<DEMO_NODE> getNodesByParentId(int pId){
        db=SQLiteDatabase.openOrCreateDatabase(DBManager.DB_PATH + "/" + DBManager.DB_NAME, null);
